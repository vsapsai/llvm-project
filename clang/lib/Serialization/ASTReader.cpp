...
InputFile ASTReader::getInputFile(ModuleFile &F, unsigned ID, bool Complain) {
  ...
  if (!Overridden && FileChange.Kind != Change::None) {
    if (Complain) {
      ...
      Diag(diag::note_ast_file_input_files_validation_status) << F.InputFilesValidationStatus;
    }
  ...
  while (true) {
    ...
    case llvm::BitstreamEntry::EndBlock: {
      ...
      if (!DisableValidation && F.Kind != MK_ExplicitModule &&
        ...
        F.InputFilesValidationStatus = ValidateSystemInputs ? InputFilesValidation::AllFiles : InputFilesValidation::UserFiles;
        if (HSOpts.ModulesValidateOncePerBuildSession &&
            F.InputFilesValidationTimestamp > HSOpts.BuildSessionTimestamp &&
            F.Kind == MK_ImplicitModule) {
          N = ForceValidateUserInputs ? NumUserInputs : 0;
          F.InputFilesValidationStatus = ForceValidateUserInputs ? InputFilesValidation::UserFiles : InputFilesValidation::SkippedInBuildSession;
        }
